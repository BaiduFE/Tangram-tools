...
baidu.dom.ready(function(){
	baidu.g("codeSearch").innerHTML = ['<div class="toc-outer">',
		'<div class="toc">	',
			'<form id="csform" action="' + config.codePath + '" method="POST">',
				...
				'<div>',
					'<select id="versionSelect" name="version"><option value="stable" id="versionOptStable">稳定版</option><option value="nightly" id="versionOptNightly">最新测试版</option></select>',
				'</div>',
				...
				'<input type="hidden" name="project" value="component" />',
		...
		'</div>',
	'</div>',
	'<h1 class="title" id="codesearchTitle">Tangram CodeSearch <small>1.3.2</small></h1>',
	...
	if (hashCode && hashCode[1]) {
		...
		var res = /#(nightly|stable)|/.exec(window.location.hash),
			version = res[1] || "stable",
			dataUrl = version == "nightly"? config.dataNightlyPath : config.dataStablePath;
	...
	function run() {
		...
		baidu.event.on("versionSelect", "change", function(e){
			if (confirm("切换版本会清空目前所选模块，是否继续？")) {
				...
				var prevId = this.value == "stable" ? "versionOptNightly" : "versionOptStable";
		...
		if (version == "nightly") {
			baidu.dom.g("versionOptNightly").selected = true;
		}
		...
		function _renderFunction(funcName,indent){
			...
			_html.push('<div class="inline-block select" title="选择/取消" onclick="window.cs.toggleSelectFunction(arguments[0] || window.event,\''+funcName+'\')"><img src="csres/empty.gif"></div>');
